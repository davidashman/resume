...
pdfmetrics.registerFont(
    TTFont("Raleway-Regular", "src/fonts/Raleway/Raleway-Regular.ttf")
)
pdfmetrics.registerFont(
    TTFont("Raleway-Italic", "src/fonts/Raleway/Raleway-Italic.ttf")
)
pdfmetrics.registerFont(
    TTFont("Raleway-SemiBold", "src/fonts/Raleway/Raleway-SemiBold.ttf")
)
...
class ResumeGenerator:
    ...
    def _setup_styles(self):
        ...
        # Biography style
        styles.add(ParagraphStyle(name="Biography", parent=styles["ResumeNormal"]))
    ...
    def _paragraph_with_larger_first_letter(self, text, style):
        ...
        for word in words:
            if len(word) > 0:
                ...
                formatted_word = (
                    f'<font size="{style.fontSize * 1.3}">{first_letter}</font>{rest}'
                )
    ...
    def _add_header(self):
        ...
        if "website" in cv:
            contact_parts.append(
                self._as_link(cv["website"], f"https://{cv['website']}")
            )
    ...
    def _add_experience(self):
        ...
        # Section header
        header = self._paragraph_with_larger_first_letter(
            "Experience", self.styles["SectionHeader"]
        )
    ...
    def _add_expertise(self):
        ...
        # Section header
        header = self._paragraph_with_larger_first_letter(
            "Core Expertise", self.styles["SectionHeader"]
        )
        self.story.append(header)

        for item in sections["core_expertise"]:
            label = Paragraph(
                f"<font name='Raleway-SemiBold'>{item['label']}: </font>{item['details']}",
                self.styles["Expertise"],
            )
    ...
    def _add_thought_leadership(self):
        ...
        # Section header
        header = self._paragraph_with_larger_first_letter(
            "Thought Leadership", self.styles["SectionHeader"]
        )
        self.story.append(header)
        ...
        for item in sections["thought_leadership"]:
            ...
            text = (
                f"<font name='Raleway-SemiBold'>{item['label']}: </font>{details_text}"
            )
    ...
    def _add_education(self):
        ...
        # Section header
        header = self._paragraph_with_larger_first_letter(
            "Education", self.styles["SectionHeader"]
        )
        self.story.append(header)
    ...
    def _add_bio(self):
        ...
        # Section header
        header = self._paragraph_with_larger_first_letter(
            "Executive Biography", self.styles["SectionHeader"]
        )
        self.story.append(header)
...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        print(
            "Example: python generate_resume_pdf.py resume.yaml my_resume.pdf bio.pdf"
        )
        sys.exit(1)
